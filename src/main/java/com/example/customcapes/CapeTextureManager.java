...
public class CapeTextureManager {
	...
	public static void clearAll() {
		...
		if (deleteTextureMethod != null) {
			Minecraft mc = Minecraft.getMinecraft();
			if (mc != null) {
				for (ResourceLocation loadedResource : loadedResources) {
					try {
						deleteTextureMethod.invoke(mc.getTextureManager(), loadedResource);
					} catch (Exception e) {
						FMLLog.log(Level.ERROR, "[CustomCapes] Failed to delete texture: %s", loadedResource);
					}
				}
			}
		}
